...
pub fn bench_set_vertices<D, T>(b: &mut Bencher, sandbox: &mut DatastoreTestSandbox<D, T>)
    where D: Datastore<T>,
          T: Transaction
{
    let trans = sandbox.transaction();
    ...
    b.iter(|| {
        let trans = sandbox.transaction();
        let q = VertexQuery::Vertex(id);
        let t = Type::new("test_vertex".to_string()).unwrap();
        trans.set_vertices(q, t).unwrap();
    });
}
...
pub fn bench_set_edges<D, T>(b: &mut Bencher, sandbox: &mut DatastoreTestSandbox<D, T>)
    where D: Datastore<T>,
          T: Transaction
{
    let trans = sandbox.transaction();

...
set_edge(id_1, "bar", id_2, 1.0);
set_edge(id_2, "bar", id_3, 1.0);
...
local vertices = get_vertices(queries.VertexQuery.all("00000000-0000-0000-0000-000000000000", 10).query)
check_vertices(vertices, 5, {[id_1]=true, [id_2]=true, [id_3]=true, [id_4]=true, [id_5]=true});

-- Ensure we can get a single vertex
local vertices = get_vertices(queries.VertexQuery.vertex(id_1).query)
check_vertices(vertices, 1, {[id_1]=true});

-- Ensure we can get a specific set of vertices
local vertices = get_vertices(queries.VertexQuery.vertices({id_1, id_2, id_3}).query)

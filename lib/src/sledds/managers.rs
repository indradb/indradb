...
impl<'db: 'tree, 'tree> VertexManager<'db, 'tree> {
    ...
    pub fn iterate_for_range<'a>(&'a self, id: u64) -> Result<impl Iterator<Item = Result<VertexItem>> + 'a> {
...
impl<'tree> EdgeRangeManager<'tree> {
    ...
    fn key(&self, first_id: u64, t: &models::Type, second_id: u64) -> Vec<u8> {
        build(&[
            Component::Id(first_id),
            Component::Type(t),
            Component::Id(second_id),
        ])
    }

    fn iterate<'it>(&self, iterator: DbIterator, prefix: Vec<u8>) -> Result<impl Iterator<Item = Result<EdgeRangeItem>> + 'it> {
    ...
    // TODO: this could be simplified by using `scan_prefix`
    pub fn iterate_for_range<'iter, 'trans: 'iter>(&'trans self, id: u64, t: Option<&models::Type>) -> Result<Box<dyn Iterator<Item = Result<EdgeRangeItem>> + 'iter>> {
        match t {
            Some(t) => {
    ...
    pub fn delete(&self, first_id: u64, t: &models::Type, second_id: u64) -> Result<()> {
        self.cf.remove(&self.key(first_id, t, second_id))?;
        Ok(())
    }
}
...
impl<'tree> VertexPropertyManager<'tree> {
    ...
    pub fn iterate_for_owner(&self, vertex_id: u64) -> Result<impl Iterator<Item = Result<OwnedPropertyItem>> + '_> {
    ...
    pub fn set(&self, vertex_id: u64, name: &str, value: &JsonValue) -> Result<()> {
        let key = self.key(vertex_id, name);
    ...
    pub fn delete(&self, vertex_id: u64, name: &str) -> Result<()> {
...
impl<'tree> EdgePropertyManager<'tree> {
    ...
    fn key(&self, outbound_id: u64, t: &models::Type, inbound_id: u64, name: &str) -> Vec<u8> {
        build(&[
    ...
    pub fn iterate_for_owner<'a>(
        ...
        let mapped = iterator.map(move |item| -> Result<EdgePropertyItem> {
            ...
            let edge_property_outbound_id = read_id(&mut cursor);
            ...
            let edge_property_inbound_id = read_id(&mut cursor);
    ...
    pub fn get(&self, outbound_id: u64, t: &models::Type, inbound_id: u64, name: &str) -> Result<Option<JsonValue>> {
    ...
    pub fn delete(&self, outbound_id: u64, t: &models::Type, inbound_id: u64, name: &str) -> Result<()> {

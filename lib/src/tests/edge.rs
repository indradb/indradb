use super::super::{Datastore, EdgeDirection, EdgeKey, Transaction, WithKeyEdgeQuery, WithIdVertexQuery, VertexQueryExt, EdgeQueryExt};
...
pub fn should_get_a_valid_edge<D: Datastore>(datastore: &mut D) {
    ...
    let e = trans.get_edges(WithKeyEdgeQuery::single(key.clone())).unwrap();
    assert_eq!(e.len(), 1);
...
pub fn should_not_get_an_invalid_edge<D: Datastore>(datastore: &mut D) {
    ...
    let e = trans.get_edges(WithKeyEdgeQuery::single(EdgeKey::new(outbound_v.id, edge_t.clone(), Uuid::default()))).unwrap();;
    assert_eq!(e.len(), 0);
    let e = trans.get_edges(WithKeyEdgeQuery::single(EdgeKey::new(Uuid::default(), edge_t.clone(), inbound_v.id))).unwrap();
    assert_eq!(e.len(), 0);
}
...
pub fn should_create_a_valid_edge<D: Datastore>(datastore: &mut D) {
    ...
    // single edge
    let e = trans.get_edges(WithIdVertexQuery::single(outbound_v.id).outbound(10)).unwrap();
    assert_eq!(e.len(), 1);
    assert_eq!(key, e[0].key);
}
...
pub fn should_delete_a_valid_edge<D: Datastore>(datastore: &mut D) {
    ...
    let key = models::EdgeKey::new(outbound_v.id, edge_t.clone(), inbound_v.id);
    trans.create_edge(&key).unwrap();
    trans.delete_edges(WithKeyEdgeQuery::single(key.clone())).unwrap();
    let e = trans.get_edges(WithKeyEdgeQuery::single(key.clone())).unwrap();
    assert_eq!(e.len(), 0);
}
...
pub fn should_not_delete_an_invalid_edge<D: Datastore>(datastore: &mut D) {
    ...
    trans.create_vertex(&outbound_v).unwrap();
    let edge_t = models::Type::new("test_edge_type".to_string()).unwrap();
    trans.delete_edges(WithKeyEdgeQuery::single(EdgeKey::new(outbound_v.id, edge_t, Uuid::default()))).unwrap();
}
...
pub fn should_get_an_edge_range<D: Datastore>(datastore: &mut D) {
    ...
    let trans = datastore.transaction().unwrap();
    let t = models::Type::new("test_edge_type".to_string()).unwrap();
    let range = trans.get_edges(WithIdVertexQuery::single(outbound_id).outbound(10).t(t).low(start_time).high(end_time)).unwrap();
...
pub fn should_get_edges_with_no_type<D: Datastore>(datastore: &mut D) {
    let (outbound_id, start_time, end_time, _) = create_time_range_queryable_edges(datastore);
    let trans = datastore.transaction().unwrap();
    let range = trans.get_edges(WithIdVertexQuery::single(outbound_id).outbound(10).low(start_time).high(end_time)).unwrap();
    check_edge_range(&range, outbound_id, 5);
}
...
pub fn should_get_edges_with_no_high<D: Datastore>(datastore: &mut D) {
    ...
    let trans = datastore.transaction().unwrap();
    let t = models::Type::new("test_edge_type".to_string()).unwrap();
    let range = trans.get_edges(WithIdVertexQuery::single(outbound_id).outbound(10).t(t).low(start_time)).unwrap();
...
pub fn should_get_edges_with_no_low<D: Datastore>(datastore: &mut D) {
    ...
    let trans = datastore.transaction().unwrap();
    let t = models::Type::new("test_edge_type".to_string()).unwrap();
    let range = trans.get_edges(WithIdVertexQuery::single(outbound_id).outbound(10).t(t).high(end_time)).unwrap();
    check_edge_range(&range, outbound_id, 10);
}
...
pub fn should_get_edges_with_no_time<D: Datastore>(datastore: &mut D) {
    ...
    let trans = datastore.transaction().unwrap();
    let t = models::Type::new("test_edge_type".to_string()).unwrap();
    let range = trans.get_edges(WithIdVertexQuery::single(outbound_id).outbound(100).t(t)).unwrap();
...
pub fn should_get_no_edges_for_reversed_time<D: Datastore>(datastore: &mut D) {
    ...
    let trans = datastore.transaction().unwrap();
    let t = models::Type::new("test_edge_type".to_string()).unwrap();
    let range = trans.get_edges(WithIdVertexQuery::single(outbound_id).outbound(10).t(t).low(end_time).high(start_time)).unwrap();
    check_edge_range(&range, outbound_id, 0);
}
...
pub fn should_get_edges<D: Datastore>(datastore: &mut D) {
    ...
    let trans = datastore.transaction().unwrap();
    let t = models::Type::new("test_edge_type".to_string()).unwrap();
    let q = WithKeyEdgeQuery::new(vec![
...
pub fn should_get_edges_piped<D: Datastore>(datastore: &mut D) {
    ...
    let query_1 = WithIdVertexQuery::single(outbound_v.id).outbound(1).t(models::Type::new("test_edge_type".to_string()).unwrap());

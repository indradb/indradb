use super::super::{BulkInsertItem, Datastore, EdgeKey, Transaction, Type, Vertex, WithIdVertexQuery, WithKeyEdgeQuery, VertexQueryExt, EdgeQueryExt};
...
pub fn should_bulk_insert<D: Datastore>(datastore: &mut D) {
    ...
    let vertices = trans.get_vertices(WithIdVertexQuery::new(vec![outbound_v.id, inbound_v.id])).unwrap();
    ...
    let edges = trans.get_edges(WithKeyEdgeQuery::single(key.clone())).unwrap();
    ...
    let vertex_properties = trans.get_vertex_properties(WithIdVertexQuery::single(outbound_v.id).property("vertex_property_name")).unwrap();
    ...
    let edge_properties = trans.get_edge_properties(WithKeyEdgeQuery::single(key.clone()).property("edge_property_name")).unwrap();

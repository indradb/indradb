...
pub fn should_handle_vertex_properties<D: Datastore>(datastore: &mut D) {
    ...
    let result = trans.get_vertex_properties(&q, &name).unwrap();
    assert_eq!(result.len(), 0);

    // Set and get the value as true
    trans.set_vertex_properties(&q, &name, &JsonValue::Bool(true)).unwrap();
    let result = trans.get_vertex_properties(&q, &name).unwrap();
    ...
    trans.set_vertex_properties(&q, &name, &JsonValue::Bool(false)).unwrap();
    let result = trans.get_vertex_properties(&q, &name).unwrap();
    assert_eq!(result.len(), 1);
    assert_eq!(result[0].id, v.id);
    ...
    trans.delete_vertex_properties(&q, &name).unwrap();
    let result = trans.get_vertex_properties(&q, &name).unwrap();
    assert_eq!(result.len(), 0);
}
...
pub fn should_not_set_invalid_vertex_properties<D: Datastore>(datastore: &mut D) {
    ...
    trans.set_vertex_properties(&q, "foo", &JsonValue::Null).unwrap();
    let result = trans.get_vertex_properties(&q, "foo").unwrap();
    assert_eq!(result.len(), 0);
}
...
pub fn should_not_delete_invalid_vertex_properties<D: Datastore>(datastore: &mut D) {
    ...
    trans.delete_vertex_properties(&q, "foo").unwrap();
...
pub fn should_handle_edge_properties<D: Datastore>(datastore: &mut D) {
    ...
    let name = format!("edge-properties-{}", generate_random_secret(8));

    trans.create_edge(&key).unwrap();

    // Check to make sure there's no initial value
    let result = trans.get_edge_properties(&q, &name).unwrap();
    assert_eq!(result.len(), 0);

    // Set and get the value as true
    trans.set_edge_properties(&q, &name, &JsonValue::Bool(true)).unwrap();
    let result = trans.get_edge_properties(&q, &name).unwrap();
    assert_eq!(result.len(), 1);
    ...
    // Set and get the value as false
    trans.set_edge_properties(&q, &name, &JsonValue::Bool(false)).unwrap();
    let result = trans.get_edge_properties(&q, &name).unwrap();
    assert_eq!(result.len(), 1);
    assert_eq!(result[0].key, key);
    assert_eq!(result[0].value, JsonValue::Bool(false));
    ...
    // Delete & check that it's deleted
    trans.delete_edge_properties(&q, &name).unwrap();
    let result = trans.get_edge_properties(&q, &name).unwrap();
    assert_eq!(result.len(), 0);
}
...
pub fn should_not_set_invalid_edge_properties<D: Datastore>(datastore: &mut D) {
    ...
    trans.set_edge_properties(&q, "bar", &JsonValue::Null).unwrap();
    let result = trans.get_edge_properties(&q, "bar").unwrap();
    assert_eq!(result.len(), 0);
}
...
pub fn should_not_delete_invalid_edge_properties<D: Datastore>(datastore: &mut D) {
    ...
    trans.delete_edge_properties(&q, "bar").unwrap();
    ...
    trans.create_edge(&key).unwrap();
    trans
        .delete_edge_properties(&EdgeQuery::Edges { keys: vec![key] }, "bleh")
        .unwrap();
}

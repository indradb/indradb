...
pub fn should_delete_indexed_vertex_property<D: Datastore>(datastore: &mut D) {
    ...
    let result = trans.get_vertices(models::PropertyPresenceVertexQuery::new("deletable-vertex-property")).unwrap();
...
pub fn should_delete_indexed_edge_property<D: Datastore>(datastore: &mut D) {
    ...
    let result = trans.get_edges(models::PropertyPresenceEdgeQuery::new("deletable-edge-property")).unwrap();
    assert_eq!(result.len(), 0);
}
...
pub fn should_update_indexed_vertex_property<D: Datastore>(datastore: &mut D) {
    ...
    let trans = datastore.transaction().unwrap();
    let q = models::SpecificVertexQuery::single(id);
    trans.set_vertex_properties(q.clone().property("updateable-vertex-property"), &json_false).unwrap();

    // property foo should not be the old value
    let result = trans.get_vertices(models::PropertyValueVertexQuery::new("updateable-vertex-property", json_true.clone())).unwrap();
    assert_eq!(result.len(), 0);
    let result = trans.get_vertices(q.clone().with_property_equal_to("updateable-vertex-property", json_true.clone())).unwrap();
    assert_eq!(result.len(), 0);
    let result = trans.get_vertices(q.clone().with_property_not_equal_to("updateable-vertex-property", json_true)).unwrap();
    assert_eq!(result.len(), 1);
    assert_eq!(result[0].id, id);

    // property foo should be the new value
    let result = trans.get_vertices(models::PropertyValueVertexQuery::new("updateable-vertex-property", json_false.clone())).unwrap();
    assert_eq!(result.len(), 1);
    assert_eq!(result[0].id, id);
    let result = trans.get_vertices(q.clone().with_property_equal_to("updateable-vertex-property", json_false.clone())).unwrap();
    assert_eq!(result.len(), 1);
    assert_eq!(result[0].id, id);
    let result = trans.get_vertices(q.with_property_not_equal_to("updateable-vertex-property", json_false)).unwrap();
    assert_eq!(result.len(), 0);
}
...
pub fn should_update_indexed_edge_property<D: Datastore>(datastore: &mut D) {
    ...
    let trans = datastore.transaction().unwrap();
    let q = models::SpecificEdgeQuery::single(key.clone());
    trans.set_edge_properties(q.clone().property("updateable-edge-property"), &json_false).unwrap();

    // property foo should not be the old value
    let result = trans.get_edges(models::PropertyValueEdgeQuery::new("updateable-edge-property", json_true.clone())).unwrap();
    assert_eq!(result.len(), 0);
    let result = trans.get_edges(q.clone().with_property_equal_to("updateable-edge-property", json_true.clone())).unwrap();
    assert_eq!(result.len(), 0);
    let result = trans.get_edges(q.clone().with_property_not_equal_to("updateable-edge-property", json_true)).unwrap();
    assert_eq!(result.len(), 1);
    assert_eq!(result[0].key, key.clone());

    // property foo should be the new value
    let result = trans.get_edges(models::PropertyValueEdgeQuery::new("updateable-edge-property", json_false.clone())).unwrap();
    assert_eq!(result.len(), 1);
    assert_eq!(result[0].key, key);
    let result = trans.get_edges(q.clone().with_property_equal_to("updateable-edge-property", json_false.clone())).unwrap();
    assert_eq!(result.len(), 1);
    assert_eq!(result[0].key, key);
    let result = trans.get_edges(q.with_property_not_equal_to("updateable-edge-property", json_false)).unwrap();
    assert_eq!(result.len(), 0);
}
...
pub fn should_query_indexed_vertex_property_empty<D: Datastore>(datastore: &mut D) {
    ...
    let result = trans.get_vertices(models::PropertyPresenceVertexQuery::new("queryable-vertex-property")).unwrap();
    assert_eq!(result.len(), 0);
}
...
pub fn should_query_indexed_edge_property_empty<D: Datastore>(datastore: &mut D) {
    ...
    let result = trans.get_edges(models::PropertyPresenceEdgeQuery::new("queryable-edge-property")).unwrap();
    assert_eq!(result.len(), 0);
}

          ...
          T: Transaction
{
    ...
    trans.set_global_metadata(name.clone(), JsonValue::Bool(true)).unwrap();

    let result = trans.get_global_metadata(name.clone());
    assert_eq!(result.unwrap(), JsonValue::Bool(true));

    // Set and get the value as false
    trans.set_global_metadata(name.clone(), JsonValue::Bool(false)).unwrap();
          ...
          T: Transaction
{
    ...
    // Set and get the value as true
    trans.set_account_metadata(sandbox.owner_id, name.clone(), JsonValue::Bool(true)).unwrap();

    let result = trans.get_account_metadata(sandbox.owner_id, name.clone());
    assert_eq!(result.unwrap(), JsonValue::Bool(true));

    // Set and get the value as false
    trans.set_account_metadata(sandbox.owner_id, name.clone(), JsonValue::Bool(false)).unwrap();

    let result = trans.get_account_metadata(sandbox.owner_id, name.clone());
    assert_eq!(result.unwrap(), JsonValue::Bool(false));

    // Delete & check that it's deleted
    trans.delete_account_metadata(sandbox.owner_id, name.clone()).unwrap();
          ...
          T: Transaction
{
    ...
    // Set and get the value as true
    trans.set_vertex_metadata(q.clone(), name.clone(), JsonValue::Bool(true)).unwrap();
    let result = trans.get_vertex_metadata(q.clone(), name.clone()).unwrap();
    assert_eq!(*result.get(&owner_id).unwrap(), JsonValue::Bool(true));

    // Set and get the value as false
    trans.set_vertex_metadata(q.clone(), name.clone(), JsonValue::Bool(false)).unwrap();
    let result = trans.get_vertex_metadata(q.clone(), name.clone()).unwrap();
    assert_eq!(*result.get(&owner_id).unwrap(), JsonValue::Bool(false));

    // Delete & check that it's deleted
    trans.delete_vertex_metadata(q.clone(), name.clone()).unwrap();
          ...
          T: Transaction
{
    ...
    // Set and get the value as true
    trans.set_edge_metadata(q.clone(), name.clone(), JsonValue::Bool(true)).unwrap();
    let result = trans.get_edge_metadata(q.clone(), name.clone()).unwrap();
    assert_eq!(*result.get(&key).unwrap(), JsonValue::Bool(true));

    // Set and get the value as false
    trans.set_edge_metadata(q.clone(), name.clone(), JsonValue::Bool(false)).unwrap();
    let result = trans.get_edge_metadata(q.clone(), name.clone()).unwrap();
    assert_eq!(*result.get(&key).unwrap(), JsonValue::Bool(false));
          ...
          T: Transaction
{
    ...
    let q = EdgeQuery::Edge {
        key: EdgeKey::new(Uuid::default(), Type::new("foo".to_string()).unwrap(), Uuid::default())
    };
          ...
          T: Transaction
{
    ...
    trans.create_edge(key.clone(), weight).unwrap();
    trans.delete_edge_metadata(EdgeQuery::Edge { key: key }, "bleh".to_string()).unwrap();
}

...
pub fn should_update_a_valid_edge<D: Datastore<T, I>, T: Transaction<I>, I: Id>(sandbox: &mut DatastoreTestSandbox<D, T, I>) {
	...
	let e1 = models::Edge::new(outbound_id, edge_t.clone(), inbound_id, models::Weight::new(0.5).unwrap());
	trans.set_edge(e1.clone()).unwrap();
	let e = trans.get_edge(outbound_id, edge_t.clone(), inbound_id).unwrap();
	assert_eq!(e1, e);

	// Update the edge and check
	let e2 = models::Edge::new(outbound_id, edge_t.clone(), inbound_id, models::Weight::new(-0.5).unwrap());
...
pub fn should_not_update_an_invalid_edge<D: Datastore<T, I>, T: Transaction<I>, I: Id>(sandbox: &mut DatastoreTestSandbox<D, T, I>) {
	...
	let inbound_id = trans.create_vertex(vertex_t.clone()).unwrap();
	let edge_t = models::Type::new("test_edge_type".to_string()).unwrap();
	let result = trans.set_edge(models::Edge::new(outbound_id, edge_t.clone(), I::default(), models::Weight::new(0.5).unwrap()));
	assert_eq!(result.unwrap_err(), Error::VertexNotFound);
	let result = trans.set_edge(models::Edge::new(I::default(), edge_t, inbound_id, models::Weight::new(0.5).unwrap()));
	assert_eq!(result.unwrap_err(), Error::VertexNotFound);
}

...
pub fn get_vertices((trans, q): (ProxyTransaction, VertexQuery)) -> Result<Vec<Vertex>, Error> {
    Ok(trans.trans.get_vertices(q.0)?.into_iter().map(|x| Vertex::new(x)).collect())
}
...
pub fn get_edges((trans, q): (ProxyTransaction, EdgeQuery)) -> Result<Vec<Edge>, Error> {
    Ok(trans.trans.get_edges(q.0)?.into_iter().map(|x| Edge::new(x)).collect())
}
...
pub fn set_global_metadata((trans, key, value): (ProxyTransaction, String, JsonValue)) -> Result<(), Error> {
...
pub fn get_account_metadata((trans, owner_id, key): (ProxyTransaction, Uuid, String)) -> Result<JsonValue, Error> {
    Ok(JsonValue::new(trans.trans.get_account_metadata(owner_id.0, key)?))
}

pub fn set_account_metadata((trans, owner_id, key, value): (ProxyTransaction, Uuid, String, JsonValue)) -> Result<(), Error> {
    Ok(trans.trans.set_account_metadata(owner_id.0, key, value.0)?)
}

pub fn delete_account_metadata((trans, owner_id, key): (ProxyTransaction, Uuid, String)) -> Result<(), Error> {
    trans.trans.delete_account_metadata(owner_id.0, key)?;
    Ok(())
}

pub fn get_vertex_metadata((trans, q, key): (ProxyTransaction, VertexQuery, String)) -> Result<HashMap<Uuid, JsonValue>, Error> {
...
pub fn set_vertex_metadata((trans, q, key, value): (ProxyTransaction, VertexQuery, String, JsonValue)) -> Result<(), Error> {
    Ok(trans.trans.set_vertex_metadata(q.0, key, value.0)?)
}

pub fn delete_vertex_metadata((trans, q, key): (ProxyTransaction, VertexQuery, String)) -> Result<(), Error> {
    trans.trans.delete_vertex_metadata(q.0, key)?;
    Ok(())
}

pub fn get_edge_metadata((trans, q, key): (ProxyTransaction, EdgeQuery, String)) -> Result<HashMap<EdgeKey, JsonValue>, Error> {
...
pub fn set_edge_metadata((trans, q, key, value): (ProxyTransaction, EdgeQuery, String, JsonValue)) -> Result<(), Error> {
    trans.trans.set_edge_metadata(q.0, key, value.0)?;
    Ok(())
}

pub fn delete_edge_metadata((trans, q, key): (ProxyTransaction, EdgeQuery, String)) -> Result<(), Error> {

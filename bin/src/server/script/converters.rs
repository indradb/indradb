...
use serde_json::{
    Value as ExternalJsonValue,
    Map,
    Number as JsonNumber
};
use common::ProxyTransaction as ExternalProxyTransaction;
use braid::{
    Type as ExternalType,
    EdgeKey as ExternalEdgeKey,
    Vertex as ExternalVertex,
    Edge as ExternalEdge,
    Weight as ExternalWeight,
    EdgeQuery as ExternalEdgeQuery,
...
impl<'lua> FromLua<'lua> for JsonValue {
    fn from_lua(value: Value<'lua>, l: &'lua Lua) -> LuaResult<Self> {
        match value {
            ...
            Value::Number(value) => {
                let num = JsonNumber::from_f64(value).expect("Expected to be able to create a JSON number from a float");
            ...
            Value::String(value) => {
                let value_str = match value.to_str() {
                    Ok(s) => s.to_string(),
                    Err(err) => {
                        return Err(new_from_lua_error("string", "JSON", Some(format!("the lua string is not valid utf-8: {}", err))));
                    }
                };
            ...
            Value::Table(value) => {
                    ...
                    match Self::from_lua(key, l)? {
                        ...
                        JsonValue(ExternalJsonValue::Number(key_number)) => {
                            if key_number.is_u64() {
                                ...
                                return Err(new_from_lua_error("table key", "JSON map key", Some("the table contains an invalid key".to_string())));
                            }
                        },
                        _ => {
                            return Err(new_from_lua_error("table key", "JSON map key", Some("the table contains an invalid key".to_string())));
                ...
                if is_array {
                    ...
                    if vec.len() == 1 && vec.get(0) == Some(&ExternalJsonValue::String("__braid_json_null".to_string())) {
    ...
    fn to_lua(self, l: &'lua Lua) -> LuaResult<Value<'lua>> {
        match self.0 {
            ...
            ExternalJsonValue::Number(value) => {
                let value_float = value.as_f64().expect("Expected to be able to create a float from a JSON number");
            ...
            ExternalJsonValue::Array(value) => {
                let mapped = value.into_iter().map(|x| JsonValue::new(x));
...
impl<'lua> ToLua<'lua> for EdgeKey {
    fn to_lua(self, l: &'lua Lua) -> LuaResult<Value<'lua>> {
        let table = l.create_table();
        table.set("outbound_id", l.create_string(&self.0.outbound_id.hyphenated().to_string()[..]))?;
        table.set("type", self.0.t.0)?;
        table.set("inbound_id", l.create_string(&self.0.inbound_id.hyphenated().to_string()[..]))?;
        Ok(Value::Table(table))
    }
}
...
impl<'lua> ToLua<'lua> for Edge {
    fn to_lua(self, l: &'lua Lua) -> LuaResult<Value<'lua>> {
        ...
        table.set("weight", Value::Number(self.0.weight.0 as Number))?;
        table.set("created_datetime", l.create_string(&self.0.created_datetime.to_string()[..]))?;
        Ok(Value::Table(table))
    }
}
...
impl<'lua> FromLua<'lua> for Weight {
    fn from_lua(value: Value<'lua>, l: &'lua Lua) -> LuaResult<Self> {
        ...
        match ExternalWeight::new(value_f32) {
            ...
            Err(err) => Err(new_from_lua_error("number", "weight", Some(format!("{}", err))))
        }
    }
}
...
impl<'lua> ToLua<'lua> for Weight {
    fn to_lua(self, _: &'lua Lua) -> LuaResult<Value<'lua>> {
        Ok(Value::Number((self.0).0 as Number))
    }
}
...
impl<'lua> FromLua<'lua> for VertexQuery {
    fn from_lua(value: Value<'lua>, l: &'lua Lua) -> LuaResult<Self> {
        if let Value::Table(value) = value {
            ...
            if t == "all" {
                let start_id = match optional_string_from_value(get_table_value(&value, "start_id")?)? {
                    Some(start_id) => Some(ExternalUuid::from_str(&start_id[..]).map_err(|e| new_from_lua_error("string", "uuid", Some(format!("{}", e))))?),
                    None => None
                };
                ...
                if let Value::Table(ids_values) = get_table_value(&value, "ids")? {
                    ...
                    Err(new_from_lua_error("", "", Some("`vertices` attribute is not a table".to_string())))
                }
            } else if t == "pipe" {
                let edge_query = Box::new(EdgeQuery::from_lua(get_table_value(&value, "edge_query")?, l)?.0);
                let converter = converter_from_table(&value)?;
                let limit = limit_from_table(&value, l)?;
                Ok(VertexQuery::new(ExternalVertexQuery::Pipe { edge_query, converter, limit }))
            } else {
                Err(new_from_lua_error("", "", Some("Unexpected vertex query type".to_string())))
            }
        } else {
        ...
        if let Value::Table(value) = value {
            ...
            if t == "edges" {
                if let Value::Table(edges) = get_table_value(&value, "keys")? {
                    ...
                    Err(new_from_lua_error("", "", Some("`edges` attribute is not a table".to_string())))
                }
            } else if t == "pipe" {
                let vertex_query = Box::new(VertexQuery::from_lua(get_table_value(&value, "vertex_query")?, l)?.0);
                let converter = converter_from_table(&value)?;

                let type_filter = match optional_string_from_value(get_table_value(&value, "type_filter")?)? {
                    Some(type_filter_str) => {
                        let type_filter = ExternalType::new(type_filter_str.to_string()).map_err(|e| new_from_lua_error("string", "type", Some(format!("{}", e))))?;
                        Some(type_filter)
                    },
                    None => None
                };

                let high_filter = optional_datetime_from_value(get_table_value(&value, "high_filter")?)?;
                let low_filter = optional_datetime_from_value(get_table_value(&value, "low_filter")?)?;
                let limit = limit_from_table(&value, l)?;
                Ok(EdgeQuery::new(ExternalEdgeQuery::Pipe { vertex_query, converter, type_filter, high_filter, low_filter, limit }))
            } else {
                Err(new_from_lua_error("", "", Some("Unexpected edge query type".to_string())))
            }
        } else {
...
fn get_table_value<'lua>(table: &'lua Table, name: &str) -> LuaResult<Value<'lua>> {
    table.get(name).map_err(|_| new_from_lua_error("", "", Some(format!("missing `{}` in the table", name))))
}

fn string_from_value(value: Value) -> LuaResult<String> {
    if let Value::String(value) = value {
        let value_string = value.to_str().map_err(|e| new_from_lua_error("lua string", "native string", Some(format!("{}", e))))?;
...
fn optional_datetime_from_value<'lua>(value: Value)  -> LuaResult<Option<DateTime<Utc>>> {
...
fn uuid_from_value(value: Value) -> LuaResult<ExternalUuid> {
    ...
    let value_uuid = ExternalUuid::from_str(&value_string[..]).map_err(|e| new_from_lua_error("string", "uuid", Some(format!("{}", e))))?;
    Ok(value_uuid)
}
...
fn converter_from_table(table: &Table) -> LuaResult<QueryTypeConverter> {
    match &string_from_value(get_table_value(&table, "converter")?)?[..] {
        ...
        _ => Err(new_from_lua_error("", "", Some("Unexpected converter type".to_string())))
    }
}
...
fn limit_from_table<'lua>(table: &'lua Table, l: &'lua Lua) -> LuaResult<u32> {
    ...
    if limit < 0 {
        Err(new_from_lua_error("integer", "limit", Some("value is below 0".to_string())))
    } else {
        Ok(limit as u32)
    }
}
